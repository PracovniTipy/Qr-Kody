-- StůlHraje – Etapa 4 (hra 5, poslední hra se skóre): "Rozbíjení lahví"
-- (arkanoid/breakout styl, hospodský vizuál). Stejně jako u předchozích
-- čtyř her (migrace 0009–0012) je hra nekonečná a anti-cheat (kapitola 9.1)
-- proto odvozuje maximální možné skóre od uplynulého času místo pevné
-- konstanty. Skóre roste o 1 bod za každou rozbitou lahev/sklenici – i při
-- velmi zručném hraní realisticky nejde trefovat víc než zhruba 2 lahve za
-- sekundu, takže max_score = elapsed_seconds * 2 + rezerva je bezpečná
-- (permisivní) horní hranice.
...
begin
  ...
  if v_session.game_id = 'kosik' then
    ...
    v_max_seconds := 1800;
    v_max_score := ceil(v_elapsed_seconds * 1) + 5;
  elsif v_session.game_id = 'breakout' then
    v_min_seconds := 0.5;
    v_max_seconds := 1800;
    v_max_score := ceil(v_elapsed_seconds * 2) + 10;
  else
